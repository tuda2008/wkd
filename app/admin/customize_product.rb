ActiveAdmin.register CustomizeProduct do

  permit_params :title, :intro, :keywords, :description, {images:[]}, :visible

  menu priority: 2, label: "定制中心"
  ...
  scope("显示在定制页Y") { |product| product.visible }
  scope("不显示在定制页N") { |product| product.invisible }

  index do
    selectable_column
    id_column
    column(:title, sortable: false) { |product| link_to product.title, admin_customize_product_path(product) }
